...
dy(1,1) = y(2);
dy(2,1) = (1/D)*(-m^2*L^2*g*Cy*Sy + m*L^2*(-1*m*L*y(4)^2*Sy - d*y(2))) + m*L*L*(1/D)*u;
dy(3,1) = y(4);
dy(4,1) = (1/D)*(-1*(m+M)*m*g*L*Sy + m*L*Cy*(m*L*y(4)^2*Sy - d*y(2))) + m*L*Cy*(1/D)*u +.01*randn;
...
function [t,y] = pole_place_cart_pendulum(m, M, L, g, y_setpoint, y0)
  ...
  eigs = [-2.0; -2.8; -2.2; -2.3];                           ## Initialise desired eigenvalues
...
function cart_pendulum_main()
  ...
  L = 2;
  g = 9.8;
  #y0 = [-4; 0; pi + 0.8; 0]; ##for pole placement and lqr
  y0 = [0; 0; pi - 0.8; 0];   ##for zero input
  y_setpoint = [0; 0; pi; 0];
